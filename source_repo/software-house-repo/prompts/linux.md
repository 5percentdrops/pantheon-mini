...
Start by running `uname -s`. If output is not `Linux`: say "This is the Linux version. Grab the Mac or Windows version from https://github.com/[YOUR-GH-USERNAME]/software-house" and stop. If `Linux`: say "Linux confirmed. Let's build Software House." and proceed.

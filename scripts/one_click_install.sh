#!/usr/bin/env bash
#
# one_click_install.sh — Mini Software House V8.10
#
# Workspace + V7 baseline validators + V8.5–V8.10 ports adapted to Mini's
# 12-active-agent roster. Uses ~/.hermes-mini-<slug>/ namespace so it
...
#   bash scripts/one_click_install.sh --no-bootstrap        # skip 12 per-agent homes
...
echo "==================================================================="
echo " Mini Software House V8.10 — Full Installer"
echo "==================================================================="
echo " Paperclip = company/control plane"
echo " Hermes    = per-agent harness (12 ~/.hermes-mini-<slug>/ homes)"
...
if [ "$SKIP_BOOTSTRAP" = "0" ]; then
    echo "==> Step 5/8: Bootstrap 12 per-agent Hermes homes (~/.hermes-mini-<slug>)"
    python3 - <<'PYEOF'
...
for a in agents:
    ...
    if not cfg.exists():
        cfg.write_text(textwrap.dedent(f"""\
            # Generated by Mini Software House V8.10 bootstrap
            # Agent: {slug} ({a['name']})
            model:
              name: {a['model']}
            toolsets:
    ...
    usr = home / "USER.md"
    if not usr.exists():
        usr.write_text("# USER\n\nMain contact: Arthur — Project Manager (Mini Software House)\n")
    created += 1
...
echo "   ~/.hermes-mini-<slug>/                — 12 per-agent Hermes homes"
...
echo " Smoke ramp:                 SMOKE_SCALE.md (2 -> 12 agents)"
